...
class TradingModel:
    ...
    def analyze_sentiment(self, text):
        ...
        # Predict sentiment
        prediction = self.sentiment_model.predict(padded_sequence)
    ...
    def _save_completed_candlestick(self, candle):
        """Saves the completed candlestick data to the CSV file."""
        timestamp, open_price, high_price, low_price, close_price = candle
        with open('completed_candlesticks.csv', 'a') as f:
            f.write(f"{datetime.datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d %H:%M:%S')},{open_price},{high_price},{low_price},{close_price}\n")

    def build_and_train_sentiment_model(self, articles, input_dim, output_dim=64, input_length=100):
        """Builds and trains a sentiment analysis model."""
        X, y, tokenizer = self.data_manager.preprocess_text_data(articles, max_vocab=input_dim, max_len=input_length)
        ...
        self.tokenizer = tokenizer  # Save tokenizer for later use
        
        # Optionally, save your model and tokenizer
        model.save('sentiment_model.h5')
        with open('tokenizer.pickle', 'wb') as handle:
            pickle.dump(tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)
    def load_or_train_sentiment_model(self):
        """Loads the sentiment model and tokenizer if they exist, otherwise trains a new model."""
        model_path = 'sentiment_model.h5'
        tokenizer_path = 'tokenizer.pickle'
        
        # Attempt to load the sentiment model
        if os.path.exists(model_path):
            ...
            if os.path.exists(tokenizer_path):
                ...
                print("Tokenizer not found. You may need to retrain the model.")
        else:
            print("Sentiment model not found. Training a new model...")
            # Assuming `fetch_and_prepare_data` fetches or prepares your labeled dataset
            articles = self.data_manager.fetch_and_prepare_data()  
            # Assuming `build_and_train_sentiment_model` trains the model and also sets `self.tokenizer`
            self.build_and_train_sentiment_model(articles, input_dim=5000, output_dim=64, input_length=100)  # Adjust these parameters as needed
            
            # Save the trained model and tokenizer for future use
            self.sentiment_model.save(model_path)
